<nav class="nav_element">
    <div class="nav_anemo">
        <a href="Anemo.html"> <!--link page perso-->
            <div class="nav_icone_anemo">
                <img src="../../ressources/anemo.png" alt="">   
            </div>
            <p>Anémo</p>
        </a>
    </div>
       
    <div class="nav_cryo">
        <a href="Cryo.html"><!--link page perso-->
        <div class="nav_icone_cryo">
            <img src="../../ressources/cryo.png" alt="">
            </div>
            <p>Cryo</p>
        </a>
    </div>
    <div class="nav_electro">
        <a href="Electro.html"><!--link page perso-->
        <div class="nav_icone_electro">
            <img src="../../ressources/electro.png" alt="">
        </div>
        <p>Electro</p>
    </a>
        </div>
        <div class="nav_geo">
            <a href="Geo.php"><!--link page perso-->
            <div class="nav_icone_geo">
                <img src="../../ressources/geo.png" alt="">
            </div>
            <p>Géo</p>
        </a>
        </div>
        <div class="nav_hydro">
            <a href="Hydro.html"><!--link page perso-->
            <div class="nav_icone_hydro">
                <img src="../../ressources/hydro.png" alt="">
            </div>
            <p>Hydro</p>
        </a>
        </div>
        <div class="nav_pyro">
            <a href="Pyro.html"><!--link page perso-->
            <div class="nav_icone">
                <img src="../../ressources/Pyro.png" alt="">
            </div>
            <p>Pyro</p>
        </a>
        </div>
</nav>